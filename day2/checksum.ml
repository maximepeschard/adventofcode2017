(* Compute the difference between the largest and the smallest elements
 * of a row *)
let row_diff row =
...
(* Compute the result of the division between the two evenly divisible elements
 * of a row *)
let rec row_div row =
  ...
  | hd::tl ->
      match search_div hd tl None with
      | None -> row_div tl
      | Some n -> n

(* Compute the checksum of a spreadsheet using a given row function *)
